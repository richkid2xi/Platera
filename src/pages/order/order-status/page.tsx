...
const orderSteps = [
  { id: 'NEW', label: 'Order Placed', icon: 'ri-check-line' },
...
export default function OrderStatus() {
  ...
  const { currentOrder, dispatch } = useOrder();
  const displayItems = currentOrder?.items || [];
  ...
  useSocketRoom(
    ...
    {
      'order:status_updated': (data: { status: string }) => {
        socketConnectedRef.current = true;
        if (data.status && data.status !== currentOrder?.status) {
          dispatch({ type: 'UPDATE_ORDER_STATUS', payload: data.status as any });
        }
      },
    }
  );

  // Polling fallback: poll every 10s if socket misses an update
  useEffect(() => {
    if (!currentOrder || !currentOrder.id || currentOrder.status === 'SERVED') return;

    let isMounted = true;
    const fetchStatus = async () => {
      try {
        const response = await apiClient.get(`/public/order/${token}/orders/${currentOrder.id}/status`);
        if (isMounted && response.data.status && response.data.status !== currentOrder.status) {
          dispatch({ type: 'UPDATE_ORDER_STATUS', payload: response.data.status });
        }
      } catch (error) {
  ...
  }, [currentOrder?.id, currentOrder?.status, token, dispatch]);

  if (!currentOrder) {
    return (
      <div className="min-h-screen bg-background-50 flex flex-col items-center justify-center p-6 text-center">
        <i className="ri-error-warning-line text-4xl text-foreground-400 mb-4"></i>
        <h2 className="font-heading font-bold text-xl text-foreground-900 mb-2">No Active Order</h2>
        <p className="font-body text-sm text-foreground-500 mb-6">You haven't placed an order yet.</p>
        <Link
          to={`/order/${token}/menu`}
          className="bg-primary-500 text-white px-6 py-3 rounded-full font-heading font-bold"
        >
          Return to Menu
        </Link>
      </div>
  ...
  return (
    <div className="min-h-screen bg-background-50 pb-24 ">
      ...
      <div className="">
        <header className="px-4 pt-12 pb-4 flex items-center gap-3 max-w-2xl mx-auto w-full">
          <Link
            to={`/order/${token}/menu`}
