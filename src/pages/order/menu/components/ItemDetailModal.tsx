...
export default function ItemDetailModal({ item, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-y-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] z-50 flex items-end justify-center">
      ...
      <div className="relative w-full bg-background-50 rounded-t-3xl animate-slide-up max-h-[85vh] overflow-y-auto ">
        ...
        <div className="px-5">
          <div className="h-56 rounded-2xl overflow-hidden mt-1 bg-background-200">
            {item.image ? (
              <img
                src={item.image}
                alt={item.name}
              ...
              <div className="w-full h-full flex items-center justify-center">
                <i className="ri-restaurant-line text-4xl text-foreground-300"></i>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="px-5 pt-4 pb-6 space-y-5">
          ...
          <button
            onClick={item.available ? handleAddToCart : undefined}
            disabled={!item.available}
            className={`w-full py-4 rounded-2xl font-heading font-bold text-base transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-2 ${
              !item.available
                ? 'bg-background-200 text-foreground-400 cursor-not-allowed'
                : addedAnimation
                ? 'bg-accent-500 text-white animate-cart-ripple animate-haptic-press'
                : 'bg-primary-500 text-white hover:bg-primary-600 animate-haptic-press'
            }`}
          >
            {!item.available ? (
              <>
